...
BEGIN_OBJECT_ENUMERATION
	...
	CDIO			DI_RF_SHIELD_COVER4	            (_TEXT("DI_RF_SHIELD_COVER4"		));
	...
	CDIO			DI_SOURCE_RFG_LEAK_SENSOR		(_TEXT("DI_SOURCE_RFG_LEAK_SENSOR"	));
	CDIO			DI_BIAS_RFG_LEAK_SENSOR			(_TEXT("DI_BIAS_RFG_LEAK_SENSOR"	));	
	...
	CAIO			eAO_SRF_ARC_LevelRef			(_TEXT("eAO_SRF_ARC_LevelRef"		));
	CAIO			eAO_SRF_ARC_CountLimit			(_TEXT("eAO_SRF_ARC_CountLimit"		));
	CAIO			eAO_SRF_ReStartTime			    (_TEXT("eAO_SRF_ReStartTime"		));
	CAIO			eAO_SRF_RecoverStart			(_TEXT("eAO_SRF_RecoverStart"		));
	CAIO			eAO_SRF_RecognizeCNT			(_TEXT("eAO_SRF_RecognizeCNT"		));
	CAIO			eAO_SRF_MaskTime			    (_TEXT("eAO_SRF_MaskTime"			));
	CAIO			eAO_SRF_MaskDelayTime			(_TEXT("eAO_SRF_MaskDelayTime"		));
	CAIO			eAO_SRF_OutDelayTime			(_TEXT("eAO_SRF_OutDelayTime"		));

	CAIO			eAO_BRF_ARC_LevelRef			(_TEXT("eAO_BRF_ARC_LevelRef"		));
	CAIO			eAO_BRF_ARC_CountLimit			(_TEXT("eAO_BRF_ARC_CountLimit"		));
	CAIO			eAO_BRF_ReStartTime			    (_TEXT("eAO_BRF_ReStartTime"		));
	CAIO			eAO_BRF_RecoverStart			(_TEXT("eAO_BRF_RecoverStart"		));
	CAIO			eAO_BRF_RecognizeCNT			(_TEXT("eAO_BRF_RecognizeCNT"		));
	CAIO			eAO_BRF_MaskTime			    (_TEXT("eAO_BRF_MaskTime"			));
	CAIO			eAO_BRF_MaskDelayTime			(_TEXT("eAO_BRF_MaskDelayTime"		));
	CAIO			eAO_BRF_OutDelayTime			(_TEXT("eAO_BRF_OutDelayTime"		));

	CAIO			cRF_SRF_RampUp_Time			    (_TEXT("cRF_SRF_RampUp_Time"		));
	CAIO			cRF_BRF_RampUp_Time				(_TEXT("cRF_BRF_RampUp_Time"		));		 

	CAIO			cRF_ARC_Level_Detection			(_TEXT("cRF_ARC_Level_Detection"	));
	CAIO			cRF_ARC_CountLimit				(_TEXT("cRF_ARC_CountLimit"			));
	CAIO			cRF_ARC_ReStartTime			    (_TEXT("cRF_ARC_ReStartTime"		));
	CAIO			cRF_ARC_RecoverStartTime	    (_TEXT("cRF_ARC_RecoverStartTime"	));
	CAIO			cRF_ARC_RecognizeCount			(_TEXT("cRF_ARC_RecognizeCount"		));
	CAIO			cRF_ARC_SignalMaskTime			(_TEXT("cRF_ARC_SignalMaskTime"		));
	CAIO			cRF_ARC_SignalMaskDelayTime		(_TEXT("cRF_ARC_SignalMaskDelayTime"	));
	CAIO			cRF_ARC_SignalOutputDelayTime	(_TEXT("cRF_ARC_SignalOutputDelayTime"	));

	CAIO			cRF_ARC_Level_ReflectDisplay	(_TEXT("cRF_ARC_Level_ReflectDisplay"	));
...
SEQ_STATUS RampUpSetTime()
{
	...
	if(PM_PROC_STS.Read(nCS) == ePM_PROC_RUNNING)
	{
		if(PR_BRFG_POWER.Read(nCS) ==0 )
		{
			...
			{										
				SRF_RampUpTime = ( cRF_SRF_RampUp_Time.Read(nCS) / PR_SRFG_POWER.Read(nCS) ) * 20000 /1000;
		...
		{
			SRF_RampUpTime = ( cRF_SRF_RampUp_Time.Read(nCS) / PR_SRFG_POWER.Read(nCS) ) * 20000 /1000;
			BRF_RampUpTime = ( cRF_BRF_RampUp_Time.Read(nCS) / PR_BRFG_POWER.Read(nCS) ) * 20000 /1000;
	...
	{
		if (Fnc_SRF_SetPower.Read(nCS) !=0) SRF_RampUpTime = ( Fnc_SRF_SetRampTime.Read(nCS) / Fnc_SRF_SetPower.Read(nCS) ) * 20000 /1000; 
		else								SRF_RampUpTime =0;					

		if (Fnc_BRF_SetPower.Read(nCS) !=0)	BRF_RampUpTime = ( Fnc_BRF_SetRampTime.Read(nCS) / Fnc_BRF_SetPower.Read(nCS) ) * 20000 /1000;
...
int CHK_RF_INTLK()
{
	int		nCS					= 0;
	int		nRF_INTLK_STS		= 0;
	int     nShildCover1, nShildCover2,nShildCover3,nShildCover4, nFeedCover;
	...
	if(DI_SOURCE_RFG_LEAK_SENSOR.Read(nCS) == eOK && DI_BIAS_RFG_LEAK_SENSOR.Read(nCS) == eOK)	nRFGWLS = eOK;
	else																						nRFGWLS = eNG;	

	sprintf(szLog, "1st Check[Generator WLS 1:OK] : %d -> SRFG Water Leak : %d  BRFG Leak Flow : %d ", nRFGWLS, DI_SOURCE_RFG_LEAK_SENSOR.Read(nCS), DI_BIAS_RFG_LEAK_SENSOR.Read(nCS));
	...
	sprintf(szLog, "5th Check[Cover 1:OK]  : %d -> FeedCover:%d ShldBox1:%d ShldBox2:%d ShldBox3:%d ShldBox4:%d", nRFCover,nFeedCover, nShildCover1,nShildCover2,nShildCover3,nShildCover4);
	Make_Log("[RF_ACTION]", "%s \n",szLog);
...
void Mon_RF_ARC_PARAMETER_SET(void *pDummy)
{
	int		nCs =0;
	double  ArcRefelct=0;
	...
	while(1)
	{
		...
		ArcRefelct=  cRF_ARC_Level_ReflectDisplay.Read(nCs)/20000;
		cRF_ARC_Level_Detection.Write(sqrt(ArcRefelct)*10, nCs); 

		eAO_SRF_ARC_LevelRef.Write(	cRF_ARC_Level_Detection.Read(nCs),		nCs);
		eAO_SRF_ARC_CountLimit.Write(cRF_ARC_CountLimit.Read(nCs),			nCs);
		eAO_SRF_ReStartTime.Write(cRF_ARC_ReStartTime.Read(nCs),			nCs);		
		eAO_SRF_RecoverStart.Write(cRF_ARC_RecoverStartTime.Read(nCs),		nCs);
		if(CheckKill()) break;
		eAO_SRF_RecognizeCNT.Write(cRF_ARC_RecognizeCount.Read(nCs),		nCs);		
		eAO_SRF_MaskTime.Write(cRF_ARC_SignalMaskTime.Read(nCs),			nCs);	
		eAO_SRF_MaskDelayTime.Write(cRF_ARC_SignalMaskDelayTime.Read(nCs),	nCs);
		eAO_SRF_OutDelayTime.Write(cRF_ARC_SignalOutputDelayTime.Read(nCs),	nCs);

		if(CheckKill()) break;
		eAO_BRF_ARC_LevelRef.Write(	cRF_ARC_Level_Detection.Read(nCs),		nCs);
		eAO_BRF_ARC_CountLimit.Write(cRF_ARC_CountLimit.Read(nCs),			nCs);
		eAO_BRF_ReStartTime.Write(cRF_ARC_ReStartTime.Read(nCs),			nCs);		
		eAO_BRF_RecoverStart.Write(cRF_ARC_RecoverStartTime.Read(nCs),		nCs);
		if(CheckKill()) break;
		eAO_BRF_RecognizeCNT.Write(cRF_ARC_RecognizeCount.Read(nCs),		nCs);		
		eAO_BRF_MaskTime.Write(cRF_ARC_SignalMaskTime.Read(nCs),			nCs);	
		eAO_BRF_MaskDelayTime.Write(cRF_ARC_SignalMaskDelayTime.Read(nCs),	nCs);
		eAO_BRF_OutDelayTime.Write(cRF_ARC_SignalOutputDelayTime.Read(nCs),	nCs);
...
void Mon_ACTION_SCREENLOCK(void *pDummy )
{
	...
	while(1)
	{
		...
		if (eDI_SRF_GenRemoteMode.Equal(ON))		SRF_REMOTE_CHK.Write(eOK);
		...
		if (eDI_BRF_GenRemoteMode.Equal(ON))		BRF_REMOTE_CHK.Write(eOK);
		...
		if(	eDI_BRF_GenFailSts.Equal(ON))			BRM_ERROR_CHK.Write(eNG);
...
BOOL OnInitComplete( const TCHAR* szArgument ) 
{
	...
	_beginthread(Mon_RF_ARC_PARAMETER_SET,		0, NULL);	_sleep(100);
